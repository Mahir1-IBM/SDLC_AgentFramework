...
def correctingFSD(
    ...
    user_proxy.initiate_chat(
        Validator, message = f"Read the FSD: {data} and thena verify it by given parameters. Return the parameters which were not satisfied by the FSD.")
    
    user_proxy.stop_reply_at_receive(Validator)
    user_proxy.send(
        "Give me a list of headings and sub headings that the generated result do not have, with a proper explaination of each point.", Validator)
